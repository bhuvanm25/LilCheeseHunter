...
# Base window size for playground viewer
WINDOW_WIDTH = 1200
WINDOW_HEIGHT = 900

# Fallback colors (used when an image is missing)
COLORS = {
    "WALL": (20, 20, 30),
    "EMPTY": (235, 85, 100),
    "GRID": (45, 45, 60),
...
# Sprite indices (for the tileset)
IDX_VOID = 1
IDX_WALL = 2
IDX_FLOOR = 3
IDX_TRAP = 4
...
class PygameViewer:
    def __init__(self, rows, cols, max_width=WINDOW_WIDTH,
                 max_height=WINDOW_HEIGHT, title="Lil Cheese Hunter™ - Playground"):
        # Basic layout / window setup
        pygame.init()
        self.rows = rows
        self.cols = cols

        # Used to detect “just ate cheese / hit trap” per step
        self.prev_cheese_mask = None
        self.prev_agent_pos = None

        # Compute tile size to fit grid within max_width/height
        margin_factor = 1.1
        ...
        # Load tile images from images/ folder
        self.tile_images = self._load_tile_images()

        # FPS slider setup (controls training speed)
        ...
        # Skip/show animation button below slider
        btn_w, btn_h = 80, 30
        btn_x = panel_x0 + (panel_w - btn_w) // 2
        btn_y = self.slider_y + 30
        self.skip_button_rect = pygame.Rect(btn_x, btn_y, btn_w, btn_h)

        # True = draw full animation, False = progress-only mode
        self.show_animation = True

        # Training report (PNG) overlay
    ...
    def _load_tile_images(self):
        # Load and scale all tile images used in the viewer
        images = {}
    ...
    def load_report_image(self, filename="training_report.png"):
        # Load a training-report PNG and scale it for overlay
        base_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    def _rect_at(self, r, c):
        # Pixel rect for grid coordinate (r, c)
    ...
    def cell_at_pixel(self, x, y):
        # Convert screen pixel to (row, col) or None if outside grid
    ...
    def _value_to_slider_x(self, value: int) -> int:
        # Map FPS value to a slider knob x-position
    ...
    def _pos_to_value(self, px: int) -> int:
        # Map slider knob x-position back to FPS value
    ...
    def _handle_slider_event(self, event):
        # Handle mouse interaction with the FPS slider
    ...
    def _draw_slider_and_button(self):
        # Draw FPS slider + SKIP/SHOW toggle
    ...
    def _draw_report_overlay(self):
        # Dark overlay + big training-report image + close box
    ...
    def _draw_stats_panel(self, env):
        # Right-side panel: training stats / run stats / hints
        panel_x0 = self.grid_width
        ...
        title = self.font.render("Lil Cheese Hunter™ Stats", True, (255, 255, 255))
        ...
        # Report button (if we have a generated PNG)
        ...
        # Hyper-parameters / training config from env
        ...
        if in_training:
            ...
            # Overall progress bar across all tiles
            if total_tiles > 0:
            ...
            # Non-training: show current run stats
        ...
        # Little notices about skip mode and slider
    ...
    def _draw_progress_only(self, env):
        # Training fast mode: no tile animation, just stats panel
    ...
    def draw(self, env, handle_events=True, fps=None):
        # Draw one frame; returns False if window is closed
        if getattr(env, "in_training", False) and not self.show_animation:
            # “Skip” mode → only stats + overlay
        ...
        if handle_events:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    # Open report overlay via REPORT button
                    if (
        ...
        # Detect if agent just consumed a cheese
        ...
        # Draw agent (Jerry) with context-specific sprite
        ...
        # Brief pause on cheese/trap event so animation is visible
        if just_ate_cheese or just_hit_trap:
            pygame.time.delay(200)

        self.clock.tick(self.fps_value)

        # Save for next frame’s diff checks
