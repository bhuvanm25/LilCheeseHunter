...
# Window size for the Map Tools menu
...
def map_menu_screen() -> str:
    # Main "Map Tools" screen with three options: editor, viewer, back
    pg.init()
    screen = pg.display.set_mode((APP_W, APP_H))
    pg.display.set_caption("Lil Cheese Hunter™ - Map Tools")
    ...
    # Default fallback choice if window closes
    choice = "back"
    running = True
    ...
    while running:
        ...
        hint = font_small.render(
            "Create maps or view existing ones.",
            True,
            TEXT_SUB,
        )
        screen.blit(hint, (pad_x, 120))

        # Small helper to draw a button with hover effect
...
def main():
    # Loop until user chooses "back" from the map tools
    while True:
        ...
        if choice == "editor":
            ...
            # Open viewer launcher
            viewer.run_picker_and_view()

        else:
            # "back" or window close → exit
            break
