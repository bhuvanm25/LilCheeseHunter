...
# Minimal JSON structure we expect a map file to have
EXPECTED_KEYS = {"w", "h", "grid"}


def is_map_json(path):
    # Quick validation that a file looks like one of our map JSONs
    try:
...
# Folders we scan for maps in the picker
SEARCH_DIRS = ["cleaned", "raw", "."]


def list_maps():
    # Find all JSON maps in cleaned/, raw/, and current folder
    results = []
    ...
    # Fallback: scan current folder alone
    if not results:
...
def load_map(path):
    # Load and normalize a map file into width, height, and grid
    with open(path, "r") as f:
        data = json.load(f)
    w, h, g = data["w"], data["h"], data["grid"]

    # Trim rows/cols to the stored w/h
    ...
    for row in g:
        new_row = []
        for v in row:
            # Replace unknown tile IDs with empty
            if v not in PALETTE:
                v = 0
            new_row.append(v)
...
class Viewer:
    # Simple map viewer for a single JSON map
    def __init__(self, map_path):
        ...
        pg.display.set_caption(f"Lil Cheese Hunter™ - Viewer — {title_name}")

        # Layout: map on left, sidebar on right
        self.side_w = 260
        ...
        # Clickable "Back" button rect in sidebar
        self.back_rect = None

        # Start zoomed to show the whole map
        self.fit_to_view()

    def world_size_px(self):
        # World size in pixels with current tile size
        return self.w * self.tile_px, self.h * self.tile_px

    def clamp_cam(self):
        # Clamp camera so you can't scroll beyond map edges
    ...
    def fit_to_view(self):
        # Choose a tile size so the whole map fits the viewport
        vw, vh = self.viewport.size
    ...
    def zoom_at(self, mx, my, factor):
        ...
        # Convert screen point to world coords, then rescale camera
    ...
    def draw_side(self):
        # Sidebar on the right: info, controls, legend, Back button
    ...
    def draw_map(self):
        # Draw the visible portion of the map + grid
            ...
            for e in pg.event.get():
                if e.type == pg.QUIT:
                    ...
                    if e.key == pg.K_ESCAPE:
                        ...
                        # Reset to default zoom at top-left
...
def run_picker_and_view():
    # Launch file picker, then open viewer for chosen map
    items = list_maps()
    if not items:
    ...
    pg.display.set_caption("Lil Cheese Hunter™ - Map Viewer")
    ...
    while running:
        ...
        title = font.render(
            "Select a map (Up/Down, Enter, Esc/Back)",
            True,
            (240, 240, 250),
        )
        ...
        for e in pg.event.get():
            if e.type == pg.QUIT:
                ...
                # Click on a row to move selection
...
if __name__ == "__main__":
    # If given a path, open viewer directly; else use picker
